...
| `offline_model.py` | Offline stand-in for the Gemini chat model and embeddings, used only when no API key is set |
...
`app_demo.py` swaps in `offline_model.py` and builds a FAISS index with local embeddings in
